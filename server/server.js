...
io.on('connection', (socket) => {
    ...
    socket.on('join', (parms, callback) =>{
        ...
        io.to(parms.room).emit('updateUserList', users.getUserList(parms.room));

        socket.emit('newMessage', generateMessage('admin', 'Welcome to the chat app'));

        socket.broadcast.to(parms.room)
            .emit('newMessage',generateMessage('admin', `${parms.name} has joined.`));
